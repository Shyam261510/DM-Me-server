...
export class Processors {
  ...
  async addDataToRedisProcessor(job: Job<AddDataToRedisQueue>) {
    const { key, value } = job.data;

    const res = await redis.set(key, value);
    if (!res.success) {
      console.error("addUserToRedisProcessor error:", res.message);
      return { success: false, message: res.message };
    }
    console.log("User added to redis successfully with key: ", key);
    return { success: true, message: "User added to redis successfully" };
  }
}
